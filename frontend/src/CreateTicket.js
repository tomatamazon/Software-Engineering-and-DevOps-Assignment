...
async function postCreateTicket(ticketTitle, ticketDescription, ticketAssignee, ticketProgressEstimate) {
    const response = await fetch("/post_data", {
        ...
        body: JSON.stringify({ "title": ticketTitle, "description": ticketDescription, "assignee": ticketAssignee, "progress_estimate": ticketProgressEstimate })
    });
    ...
    const handleCreateTicketEvent = async () => {
        if (ticketTitle === "" || ticketDescription === "" || ticketAssignee === "") {
            ...
            if (databasePostStatus === "success") {
                ...
                props.setCreateTicketState(false);
                props.setCreateTicketFailureState(false);
            };
        };
    };
        ...
        <div>
            <div className="create-ticket-parent">
                <div classname="return-to-home-page">
                    <Button variant="danger" onClick={returnToHomePage}>Exit without saving</Button>
                </div>
                <div className="create-ticket-title">
                    <label>
                        Title:
                        <Form.Control style={{ 'width': '800px' }} placeholder="Describe your ticket..." onChange={handleTicketTitle} />
                    </label>
                </div>
                <div className="create-ticket-description">
                    <label>
                        Description:
                        <Form.Control as="textarea" rows={3} style={{ 'width': '800px' }} onChange={handleTicketDescription} />
                    </label>
                </div>
